    ...
    <div className="donut-row">
      <div className="donut-wrap" style={{ width: size, height: size }}>
        <svg
          ...
          role="img"
          aria-label="Répartition du stock"
        >
          <circle
            cx={size / 2}
            cy={size / 2}
      ...
      <div className="donut-legend">
        {slices.map((s, i) => (
          <div key={s.name} className="dl-item">
            <span
              className="dl-dot"
              style={{
                background: `var(${SLICE_VARS[i % SLICE_VARS.length]})`,
              }}
            />
        ...
        {!slices.length && (
          <span className="muted">Aucun article en stock.</span>
        )}
      </div>
    </div>
  );
    ...
    <ol className="pipe">
      {steps.map((s, i) => (
        <li
          ...
          onClick={() =>
            navigate(`/bons-passager?status=${STEP_STATUS[s.key] || ""}`)
          }
          role="button"
          tabIndex={0}
          onKeyDown={(e) => {
...
export function RouteMap({ inTransit = 0 }) {
  return (
    <div className="routemap">
      <svg
        viewBox="0 30 720 130"
        className="routemap-svg"
        role="img"
        aria-label="Route Chine — Algérie"
      >
        {/* The backdrop is the real dotted world map, drawn behind this SVG by
            .routemap in theme/shell.css — invented land shapes filled with a dot
            pattern were a drawing of a map rather than one. */}
        ...
        <g className="rm-plane rotate-283">
          <animateMotion
            dur="9s"
