...
export async function financialSummary({ period = 'mois', currency = 'DZD' } = {}, db = getPool()) {
  ...
  const [movements, byType, persons] = await Promise.all([
    ...
    db.query(
      `SELECT
         COALESCE(SUM(-balance) FILTER (WHERE person_type = 'fournisseur' AND balance < 0), 0)::text AS creances,
         COALESCE(SUM(balance)  FILTER (WHERE person_type = 'passager'    AND balance > 0), 0)::text AS dettes
  ...
  const [caisse, opening, lines] = await Promise.all([
    ...
    db.query(
      `SELECT t.id, t.created_at, t.type, t.direction, t.amount::text, t.balance_after::text,
              t.note, a.full_name AS admin_name
...
// ── 3. Relevé de compte (fournisseur / passager) ─────────────────────
export async function personStatement({ personType, personId, currency = 'DZD' } = {}, db = getPool()) {
  ...
  const [person, balance, entries] = await Promise.all([
    db.query(`SELECT id, ${nameCol} AS name, phone FROM ${table} WHERE id = $1`, [personId]),
    db.query(
    ...
    db.query(
      `SELECT pl.created_at, pl.type, pl.amount::text, pl.balance_after::text, pl.note,
              b.reference AS bon_reference, o.reference AS order_reference, a.full_name AS admin_name
...
export async function lossesReport({ period = 'mois' } = {}, db = getPool()) {
  const [lines, byResponsible] = await Promise.all([
    db.query(
      `SELECT b.reference, b.created_at, f.name AS fournisseur_name, p.full_name AS passager_name,
