...
try {
  ...
  const f = (await china.query(
    "INSERT INTO people (name, phone, is_fournisseur) VALUES ('Fourn Chine','+86 500',true) RETURNING id"
  )).rows[0];
  chk('id personne dans la plage Chine', f.id >= OFFSET.china, true);
  ...
  await china.query("INSERT INTO person_ledger (person_type, person_id, currency_code, amount, balance_after, type, admin_id, ref_bon_id) VALUES ('personne',$1,'DZD','-8000','-8000','transport_fee',1,$2)", [f.id, b.id]);
  ...
  const af = (await algeria.query('SELECT name, is_fournisseur FROM people WHERE id=$1', [f.id])).rows[0];
  chk('Algérie voit la personne', af?.name, 'Fourn Chine');
  chk('Algérie voit son rôle fournisseur', af?.is_fournisseur, true);
  ...
  const pb = (await algeria.query("SELECT balance FROM person_balances WHERE person_type='personne' AND person_id=$1 AND currency_code='DZD'", [f.id])).rows[0];
  chk('solde recalculé chez Algérie', pb?.balance, '-8000.00');
  ...
  await sync.applyEvents(algeria, pulled);
  await algeria.query('COMMIT');
  const dupF = (await algeria.query('SELECT COUNT(*)::int n FROM people WHERE id=$1', [f.id])).rows[0].n;
